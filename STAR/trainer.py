...
def train_worker(world_rank, world_size, nodes_size, args):
    ...
    if args.pretrained_weight is not None:
        ...
        # torch.onnx.export(
        #     net,                  # PyTorch Model
        #     sample_input.float().to(config.device),                    # Input tensor
        #     '../star_mini.onnx',        # Output file (eg. 'output_model.onnx')
    ...
    for i_epoch, epoch in enumerate(range(config.max_epoch + 1)):
        try:
            ...
            if epoch >= start_epoch:
                ...
                if epoch != start_epoch:
                    utility.forward_backward(config, train_loader, net_module, net, net_ema, criterions, optimizer, epoch, teacher_net)
