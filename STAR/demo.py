...
def draw_pts(img, pts, mode="pts", shift=4, color=(0, 255, 0), radius=3, thickness=3, save_path=None, dif=0,
...
def process(input_image, faces):
    ...
    # else:
    #     print("Fount {} faces".format(num_faces))
    ...
    for face in faces:
        ...
        center_w = w * face[1]
        center_h = h * face[2]
        scale = (max(face[3]*w, face[4]*h) + 0) / 200
...
if __name__ == '__main__':
    ...
    # face_file_path = '../Datasets/test_images/image_0001.png'
    ...
    for i, face_file_path in enumerate(glob.glob('../Datasets/test_images/*.png')):

        try:
            with open('../Datasets/test_results/yolo_result/' + os.path.basename(face_file_path)[:-3] + 'txt', 'r') as f:
                s = f.read()
                lines = s.split('\n')[:-1]
                faces = []
                for line in lines:
            ...
            with open('../Datasets/test_results/test_data_v2/'+ os.path.basename(face_file_path)[:-3] + 'txt', 'w') as f:
                pass
            continue
        ...
        image = cv2.imread(face_file_path)
        image_draw, results = process(image, faces)

        img = cv2.cvtColor(image_draw, cv2.COLOR_BGR2RGB)
        plt.imshow(img)
        plt.savefig('../Datasets/test_results/images/'+ os.path.basename(face_file_path))
