...
def Dwsconv2d(input, depthwise_weight, pointwise_weight, depthwise_bias=None, pointwise_bias=None,
                               stride=1, padding=0):
    # 深度卷积
    depthwise_conv = F.conv2d(input, depthwise_weight, bias=depthwise_bias, stride=stride, padding=padding,
                              groups=input.shape[1])

    # 逐点卷积
    pointwise_conv = F.conv2d(depthwise_conv, pointwise_weight, bias=pointwise_bias, stride=1, padding=0)
...
class E2HTransform(nn.Module):
    ...
    def forward(self, edgemaps):
        ...
        # return Dwsconv2d(edgemaps, depthwise_weight=self.weight, pointwise_weight=self.weight, pointwise_bias=self.bias)
...
class StackedHGNetV2(nn.Module):
    def __init__(self, config, classes_num, edge_info,
                 nstack=2, nlevels=4, in_channel=64, increase=0,
        ...
        if self.add_coord:
            ...
            convBlock = ConvBlock(3, 64, 3, 2, bn=True, relu=True)
