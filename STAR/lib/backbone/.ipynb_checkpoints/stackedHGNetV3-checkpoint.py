...
from .core.coord_conv_v2 import CoordConvTh
...
class DwsConvBlock(nn.Module):
    def __init__(self, inp_dim, out_dim, kernel_size=3, stride=1, bn=False, relu=True, groups=1):
        ...
        self.inp_dim = inp_dim
        self.depthwise_conv = nn.Conv2d(inp_dim, inp_dim, kernel_size, stride,
                                        padding=(kernel_size - 1) // 2, groups=inp_dim, bias=False)
        self.pointwise_conv = nn.Conv2d(inp_dim, out_dim, 1, 1, bias=True)
...
class ResBlock(nn.Module):
    def __init__(self, inp_dim, out_dim, mid_dim=None):
        ...
        self.conv1 = DwsConvBlock(inp_dim, mid_dim, 1, relu=False)
        self.bn2 = nn.BatchNorm2d(mid_dim)
        # self.conv2 = ConvBlock(mid_dim, mid_dim, 3, relu=False)
        self.conv2 = DwsConvBlock(mid_dim, mid_dim, 3, relu=False)
        self.bn3 = nn.BatchNorm2d(mid_dim)
        # self.conv3 = ConvBlock(mid_dim, out_dim, 1, relu=False)
        self.conv3 = DwsConvBlock(mid_dim, out_dim, 1, relu=False)
        self.skip_layer = DwsConvBlock(inp_dim, out_dim, 1, relu=False)
        if inp_dim == out_dim:
...
class E2HTransform_V2(nn.Module):
    def __init__(self, edge_info, num_points, num_edges):
        super().__init__()

        depthwise_matrix = torch.zeros([num_edges, 1, 1, 1])
        for edge_id in range(num_edges):
            depthwise_matrix[edge_id] = 1.0 

        e2h_matrix = torch.zeros([num_points, num_edges, 1, 1])
        for edge_id, isclosed_indices in enumerate(edge_info):
            is_closed, indices = isclosed_indices
            for point_id in indices:
                e2h_matrix[point_id, edge_id, 0, 0] = 1

        self.register_buffer('depthwise_weight', depthwise_matrix.float())
        self.register_buffer('pointwise_weight', e2h_matrix.float())

        print(e2h_matrix.sum(1).shape)
        bias = ((e2h_matrix @ torch.ones(num_edges, 1, 1, 1)) < 0.5).float().squeeze()
        self.register_buffer('bias', bias)

    def forward(self, edgemaps):
        depthwise = F.conv2d(edgemaps, self.depthwise_weight, groups=edgemaps.shape[1])
        return F.conv2d(depthwise, self.pointwise_weight, bias=self.bias)


class StackedHGNetV2(nn.Module):
    def __init__(self, config, classes_num, edge_info,
                 nstack=2, nlevels=4, in_channel=128, increase=0,
        ...
        if self.add_coord:
            ...
            convBlock = ConvBlock(3, 32, 7, 2, bn=True, relu=True)
        ...
        if self.cfg.use_AAM:
            ...
            self.e2h_transform = E2HTransform_V2(edge_info, self.num_points, self.num_edges)
