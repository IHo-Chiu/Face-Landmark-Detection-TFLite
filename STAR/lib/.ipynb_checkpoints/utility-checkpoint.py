...
from lib.backbone import StackedHGNetV1, StackedHGNetV2
...
def get_net(config):
    ...
    elif config.net == "stackedHGnet_v2":
        net = StackedHGNetV2(config=config,
                             classes_num=config.classes_num,
                             edge_info=config.edge_info,
                             nstack=config.nstack,
                             add_coord=config.add_coord,
...
def compute_loss(config, criterions, output, labels, heatmaps=None, landmarks=None, teacher_heatmaps=None):
    batch_weight = 1.0
    sum_loss = 0
    losses = list()
    sum_kdloss = 0
    for k in range(config.label_num):
        if config.criterions[k] in ['smoothl1', 'l1', 'l2', 'WingLoss', 'AWingLoss']:
            ...
            loss = criterions[k](heatmaps[_k], labels[k])

            if config.distill:
                p = F.log_softmax(heatmaps[_k]/config.temperature, dim=1)
                if _k == 0:
                    q = F.log_softmax(teacher_heatmaps[0]/config.temperature, dim=1)
                else:
                    q = F.log_softmax(teacher_heatmaps[3]/config.temperature, dim=1)
                kdloss = kl_loss(p, q)
        else:
            assert NotImplementedError
        ...
        if config.distill:
            kdloss = batch_weight * kdloss
                    
        sum_loss += config.loss_weights[k] * loss
        sum_kdloss += config.loss_weights[k] * kdloss
                    
        loss = float(loss.data.cpu().item())
        losses.append(loss)

    sum_loss = config.alpha*config.temperature*config.temperature * sum_kdloss + (1-config.alpha) * sum_loss
    return losses, sum_loss, sum_kdloss

kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)

def forward_backward(config, train_loader, net_module, net, net_ema, criterions, optimizer, epoch, teacher_net=None):
    ...
    for iter, sample in enumerate(train_loader):
        ...
        # loss
        losses, sum_loss, sum_kdloss = compute_loss(config, criterions, output, labels, heatmaps, landmarks, teacher_heatmaps)
        ...
        if iter % config.display_iteration == 0 or iter + 1 == len(train_loader):
            if config.logger is not None:
                losses_str = 'kdloss: {:.6f} Average Loss: {:.6f}'.format(sum_kdloss, sum(losses) / len(losses))
